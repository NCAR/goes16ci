...
Example Usage: ./build_patches.py -125.0 25 -66 50 2019-06-01T00:00:00 2019-06-01T23:59:59 goes16_build_patches_config_32.yml
"""
...
def build_patches(min_lon, min_lat, max_lon, max_lat, begin_iso_dt, end_iso_dt, config_yaml):
    ...
    if PARALLEL:
       ...
       for date in glm_file_dates:
           ...
           glm_jobs.append(client.submit(create_glm_grids, glm_path, grid_path, date, date + pd.Timedelta(file_freq), grid_freq, grid_proj_params, dx_km, x_extent_km, y_extent_km))
       ...
       for date in glm_file_dates:
           logging.info("Processing: %s", date)
           create_glm_grids(glm_path, grid_path, date, date + pd.Timedelta(file_freq), grid_freq, grid_proj_params, dx_km, x_extent_km, y_extent_km)
